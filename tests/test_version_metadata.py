...
def test_package_and_project_versions_match_release() -> None:
    ...
    assert re.search(r'^version\s*=\s*"1\.0\.0"$', pyproject, flags=re.MULTILINE)


def test_release_metadata_mentions_current_release_and_history() -> None:
    citation = (ROOT / "CITATION.cff").read_text(encoding="utf-8")
    changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    current_notes = (ROOT / "docs" / "releases" / "v1.0.0.md").read_text(encoding="utf-8")
    ...
    assert re.search(r'^version:\s*"1\.0\.0"$', citation, flags=re.MULTILINE)
